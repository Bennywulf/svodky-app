        ...
        <CardContent className="p-3">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              {job.note ? (
                <div className="mt-2 text-xs bg-white/60 rounded-xl px-2 py-1.5 text-slate-700">
                  Note: {job.note}
                </div>
              ) : null}
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                <Button
                  variant={startActive ? "default" : "outline"}
                  className="rounded-xl text-sm px-4 h-10 w-full font-semibold"
                  onClick={() => onStatusToggle(job.id, "In progress")}
                >
                  START
                </Button>
                <Button
                  variant={doneActive ? "default" : "outline"}
                  className="rounded-xl text-sm px-4 h-10 w-full font-semibold"
                  onClick={() => onStatusToggle(job.id, "Done")}
                >
                  DONE
                </Button>
                <Button
                  variant={problemActive ? "default" : "outline"}
                  className="rounded-xl text-sm px-4 h-10 w-full font-semibold"
                  onClick={() => onStatusToggle(job.id, "Problem")}
                >
                  PROBLEM
                </Button>
                <Button
                  variant={job.priorityOne ? "default" : "outline"}
                  className="rounded-xl text-sm px-4 h-10 w-full font-semibold"
                  onClick={() => onPriorityToggle(job.id)}
                >
                  P1
                </Button>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                <Button size="sm" variant="ghost" className="rounded-xl text-xs h-7" onClick={() => onMove(job.id, -1)}>
                  Up
                </Button>
                <Button size="sm" variant="ghost" className="rounded-xl text-xs h-7" onClick={() => onMove(job.id, 1)}>
                  Down
                </Button>
                <Button size="sm" variant="ghost" className="rounded-xl text-xs h-7" onClick={() => onEdit(job)}>
                  Edit
                </Button>
              </div>
            </div>
...
export default function SvodkyAppPrototype() {
  ...
  const addOrUpdateJob = () => {
    ...
    const maxPriority = Math.max(
      0,
      ...jobs.filter((job) => job.queue === activeQueue).map((job) => job.priority || 0)
    );
  ...
  const addTestJobs = () => {
    ...
    setJobs([
      ...
      },
      {
        ...
        deadline: "16.05.2026",
        machine: "Oxy 1",
        note: "test done older than 7 days - should be hidden",
        status: "Done",
        priorityOne: false,
        doneAt: oldDoneDate,
        priority: 3,
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button variant="outline" className="rounded-2xl h-11 min-w-[180px] px-5 text-sm font-semibold" onClick={exportVisibleQueueToCsv}>
              Export queue to Excel
            </Button>
            <Button
              className="rounded-2xl h-11 min-w-[180px] px-5 text-sm font-semibold"
          ...
          {queues.map((queue) => {
            ...
            return (
              <button
                ...
                className={`rounded-2xl p-3 text-left shadow-sm border transition ${
                  isActive ? "bg-slate-950 text-white border-slate-950" : "bg-white text-slate-900 border-slate-200"
                }`}
              >
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-4">
              ...
              <CardContent className="p-3 flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-slate-500">
                    Card order = production priority. P1 jobs are moved to the top automatically.
                  </p>
                  <p className="text-xs text-slate-400 mt-1">
                    Open now: {activeStats.count} jobs - {activeStats.minutes} min
                  </p>
                  <p className="text-xs text-slate-400 mt-1">Export uses the currently selected queue, filter and search.</p>
                </div>

                <div className="flex gap-2">
                  <Button variant={showMode === "open" ? "default" : "outline"} className="rounded-2xl h-11 min-w-[120px] px-5 text-sm font-semibold" onClick={() => setShowMode("open")}>
                    Open
                  </Button>
                  <Button variant={showMode === "all" ? "default" : "outline"} className="rounded-2xl h-11 min-w-[120px] px-5 text-sm font-semibold" onClick={() => setShowMode("all")}>
                    All
                  </Button>
                </div>
                ...
                <div className="relative w-full md:w-72">
                  ...
                  <Input
                    ...
                    placeholder="Search job / client / order"
                  />
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-2">
              {visibleJobs.map((job) => (
                <JobCard
                  ...
                  onMove={moveJob}
                  onPriorityToggle={togglePriorityOne}
                  onEdit={editJob}
                />
              ))}
          ...
          <div className="space-y-3">
            <Card className="rounded-2xl shadow-sm border-slate-200">
              <CardContent className="p-3">
                <h3 id="manual-form" className="font-bold text-lg text-slate-950">
                  {editingId ? "Edit job" : "Add job manually"}
                </h3>
                <p className="text-sm text-slate-500 mt-1">
                  First version copies today Excel workflow: fill in a few fields, check them and add the job to the selected queue.
                </p>
                ...
                <div className="mt-4 space-y-3">
                  <div className="grid grid-cols-[95px_1fr] gap-2">
                    <Input
                      ...
                      placeholder="ZR26-"
                      value={orderPrefix}
                      onChange={(event) => updateOrderPrefix(event.target.value)}
                    />
                    <Input
                      className="rounded-xl"
                      placeholder="Order / COMANDA, e.g. ZR26-01301"
                      value={manual.order}
                      onChange={(event) => setManual({ ...manual, order: event.target.value })}
                    />
                  </div>
                  <Input
                    className="rounded-xl"
                    placeholder="Client, e.g. VESTAS"
                    value={manual.client}
                    onChange={(event) => setManual({ ...manual, client: event.target.value })}
                  />
                  <Input
                    className="rounded-xl"
                    placeholder="Job number, e.g. 947 0711"
                    value={manual.jobNumber}
                    onChange={(event) => setManual({ ...manual, jobNumber: event.target.value })}
                  />

                  <div className="grid grid-cols-2 gap-2">
                    <Input
                      className="rounded-xl"
                      placeholder="Thickness, e.g. 20 mm"
                      value={manual.thickness}
                      onChange={(event) => setManual({ ...manual, thickness: event.target.value })}
                    />
                    <Input
                      className="rounded-xl"
                      placeholder="Minutes"
                      value={manual.minutes}
                      onChange={(event) => setManual({ ...manual, minutes: event.target.value })}
                    />
                  </div>

                  <Input
                    className="rounded-xl"
                    placeholder="Deadline, e.g. 14.05.2026"
                    value={manual.deadline}
                    onChange={(event) => setManual({ ...manual, deadline: event.target.value })}
                  />
                  <Input
                    className="rounded-xl"
                    placeholder="Machine / machine note"
                    value={manual.machine}
                    onChange={(event) => setManual({ ...manual, machine: event.target.value })}
                  />
                  <Input
                    className="rounded-xl"
                    placeholder="Note"
                ...
                <Button className="w-full rounded-2xl mt-3" onClick={addOrUpdateJob}>
                  {editingId ? "Save changes" : `Add to ${active.label}`}
                </Button>

                {editingId ? (
                  <Button variant="ghost" className="w-full rounded-2xl mt-2" onClick={resetForm}>
                    Cancel edit
                  </Button>
                ) : null}
              </CardContent>
            ...
            <Card className="rounded-2xl shadow-sm border-slate-200">
              <CardContent className="p-3">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Adds test jobs to verify counters, green DONE status, yellow P1 priority, filters and hiding of done jobs older than 7 days.
                </p>
                <div className="grid grid-cols-2 gap-2 mt-4">
                  <Button variant="outline" className="rounded-xl" onClick={addTestJobs}>
                    Add test
                  </Button>
                  <Button variant="outline" className="rounded-xl" onClick={clearAllJobs}>
                    Clear
                  </Button>
                </div>
                <Button variant="outline" className="w-full rounded-xl mt-2" onClick={exportVisibleQueueToCsv}>
